...
import { StorageFileService } from 'src/app/modulos/vinculacion/servicios/storage-file.service';
...
@Component({
  ...
  templateUrl: './crear-tarea-programada.html',
  styleUrls: ['./crear-tarea-programada.component.css'],
})
export class CrearTareaProgramadaComponent implements OnInit {
  ...
  save() {
    ...
    if(this.checkTipoActividad){
      if(!this.tarea.periodo){
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Ingrese un periodo',
        });
        return;
      }
      if(!this.tarea.fechaInicioTarea){
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
      ...
      if(!this.tarea.cantidadRepeticiones){
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Ingrese el total de actividades a crear',
        });
        return;
      }
    }
    if (this.tarea.tipoTarea == 'TAREA') {
      if(this.checkTipoTarea){
        if (
        ...
        if (!this.checkTipoActividad) {
          if (
            ...
            this.messageService.add({
              severity: 'error',
              summary: 'Error',
              detail: 'Ingrese una fecha de Entrega',
            });
            return;
          }
